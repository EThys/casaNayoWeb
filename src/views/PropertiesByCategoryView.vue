...
import { PROPERTY_FEATURES, PROPERTY_TYPE_LABELS } from '@/types/property'
...
// Category specific status (from route param)
const categoryStatus = computed(() => route.params.status as 'for-sale' | 'for-rent')

// Category Display Text
const categoryText = computed(() => {
  return categoryStatus.value === 'for-sale' 
    ? { title: 'à Vendre', subtitle: 'votre maison à acheter', tag: 'Vente Immobilière' }
    : { title: 'à Louer', subtitle: 'votre futur foyer', tag: 'Location Immobilière' }
})
...
// GSAP Animation function for reuse
const animateHero = () => {
  if (heroTitleRef.value) {
    gsap.fromTo(heroTitleRef.value, 
      { y: 100, opacity: 0 },
...
const filteredProperties = computed(() => {
  ...
  // Filter by fixed category status
  filtered = filtered.filter((p) => p.status === categoryStatus.value)
    ...
    <div class="min-h-screen bg-white overflow-x-hidden">
      ...
      <div class="relative min-h-[55vh] flex items-center overflow-hidden bg-[#1976D2] pt-40 pb-56">
        ...
        <div class="max-w-7xl mx-auto px-10 relative z-10 w-full">
          ...
          >
            ...
            <div class="text-white/60 text-[12px] font-black uppercase tracking-[0.4em]">{{ categoryText.tag }}</div>
          </div>
          ...
          >
            Maisons <br /> <span class="text-blue-200">{{ categoryText.title }}</span>
          </h1>
          ...
          <p class="text-white/70 max-w-xl text-lg font-medium leading-relaxed mb-12" 
            ...
            Parce que chaque Congolais mérite un toit de qualité. Découvrez {{ categoryText.subtitle }} parmi nos meilleures sélections à travers le pays.
          </p>
        </div>
      </div>
      ...
      <div class="max-w-7xl mx-auto px-6 md:px-10 relative z-30 -mt-24 pb-32">
        ...
        <div 
          ...
          class="bg-slate-50 border border-slate-100 rounded-[40px] p-10 mb-20 grid grid-cols-1 md:grid-cols-3 gap-12 relative overflow-hidden"
        >
          <!-- Type -->
          <div class="space-y-4">
          ...
          <div class="absolute bottom-4 right-10">
            <button @click="resetFilters" class="text-[10px] font-black text-blue-600 uppercase tracking-[0.2em] hover:scale-105 transition-all">Réinitialiser tout</button>
          </div>
        </div>
        ...
        <div v-else class="py-40 flex flex-col items-center text-center">
          ...
          <p class="text-[11px] font-bold text-slate-400 uppercase tracking-[0.3em] max-w-sm leading-loose">Nous n'avons pas encore d'offres correspondant à ces critères pour cette catégorie.</p>
          <button @click="resetFilters" class="mt-12 px-12 py-5 bg-slate-900 text-white text-[10px] font-black uppercase tracking-[0.4em] rounded-[24px] shadow-2xl shadow-slate-900/40 hover:scale-105 active:scale-95 transition-all">Tout réinitialiser</button>
        </div>
      </div>
    </div>
  </MainLayout>
